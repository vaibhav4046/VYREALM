...
The working source application is available at http://127.0.0.1:4173/ on this PC. The current source checkpoint adds format-aware original hooks and a bounded 120-variant delivery planner based on the viral-format research in `docs/VIRAL_FORMATS_RESEARCH_2026-09-07.md`. The updated package is [VYREALM-1.0.5-win-x64.exe](release/VYREALM-1.0.5-win-x64.exe). Its size, hash and packaged-file inventory are recorded in `outputs/verification/installer-1.0.5-inventory.json` after packaging. It is unsigned and has **not** been installed. An earlier installer launch in this session was rejected by automatic approval review with “blocked by policy”; the newer package does not establish installed-app verification. The older installed version is not evidence for these updates.
