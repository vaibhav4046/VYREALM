...
- Node/unit suite passes 56 tests plus standalone acceptance checks; Vite production build passes; `npm audit` reports zero known vulnerabilities. Python download tests pass. The final browser journey passes after the bundle/provenance changes, including round-trip import and reopened playback.
...
- Package current 1.0.5 source and record the read-only inventory in `outputs/verification/installer-1.0.5-inventory.json`. The package must include the format-aware hook and bounded variation planners and their tests. Do not launch it through the blocked approval path; installed-app verification remains outstanding.
