...
| Marketing Studio product templates | Implemented locally | Product route includes Studio, Lifestyle, With Model, UGC (faceless/talking-head/silent), Motion, 360 orbit, Unboxing, and Demo plans |
| Cinematic camera presets | Implemented locally | Static, pan, dolly, crane, orbit, handheld, and crash-zoom presets drive an editable FFmpeg zoom/pan graph |
| Product image or clip input | Implemented locally | Imported local image/video is stored in SQLite and required by the product adapter; missing media returns `PRODUCT_ASSET_REQUIRED` |
...
The product route is a local asset-motion equivalent: it uses the supplied product source, controlled camera movement, grade, and local audio in FFmpeg. It does not claim native neural generation or visual parity with Higgsfield until a qualified local checkpoint passes the machine and visual gates.
