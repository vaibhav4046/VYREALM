...
import { prepareYouTubeThumbnail, thumbnailUploadFailure } from './youtube-thumbnail.mjs';
...
const NETWORK_CAUSES = new Set(['ENOTFOUND','EAI_AGAIN','ECONNRESET','ECONNREFUSED','ETIMEDOUT','ERR_TLS_CERT_ALTNAME_INVALID','CERT_HAS_EXPIRED','UNABLE_TO_VERIFY_LEAF_SIGNATURE','UND_ERR_CONNECT_TIMEOUT','UND_ERR_HEADERS_TIMEOUT','UND_ERR_BODY_TIMEOUT','UND_ERR_SOCKET','UND_ERR_REQ_CONTENT_LENGTH_MISMATCH']);
function safeNetworkCause(error){for(const current of [error,error?.cause,error?.cause?.cause]){if(NETWORK_CAUSES.has(current?.code))return current.code;if(current?.message==='unexpected redirect')return 'FETCH_REDIRECT_BLOCKED';}return 'TRANSPORT_FAILED';}
...
function receipt(upload) {
  ...
  return { id: upload.id, state: upload.state, videoId: upload.videoId ?? null, channelId: upload.channelId, fileName: path.basename(upload.filePath), sha256: upload.sha256, bytesTotal: upload.size, bytesUploaded: upload.offset, createdAt: upload.createdAt, updatedAt: upload.updatedAt, privacyStatus: upload.processing?.status==='unverified'?'unverified':'private', ...(upload.processing ? {processing:upload.processing,processingVerified:upload.processing.status==='succeeded'} : {processingVerified:false}), ...(upload.thumbnail ? {thumbnail:upload.thumbnail} : {}), ...(upload.videoId ? { watchUrl: `https://www.youtube.com/watch?v=${upload.videoId}` } : {}) };
}
...
export function createYouTubeClient({ clientId = '', vault, transport = globalThis.fetch, clock = () => Date.now(), sleep = (ms, signal) => delay(ms, undefined, { signal }), chunkBytes = 8 * 1024 * 1024, maxRetries = 3, requestTimeoutMs = 45000 } = {}) {
  ...
  async function request(url, init, signal) {
    ...
    const thumbnailEndpoint=parsed.origin==='https://www.googleapis.com'&&parsed.pathname==='/upload/youtube/v3/thumbnails/set'&&init.method==='POST'&&[...parsed.searchParams.keys()].length===2&&parsed.searchParams.getAll('videoId').length===1&&VIDEO_ID.test(parsed.searchParams.get('videoId')||'')&&parsed.searchParams.getAll('uploadType').length===1&&parsed.searchParams.get('uploadType')==='media';
    const permitted = thumbnailEndpoint || url === TOKEN_URL || (parsed.origin === 'https://www.googleapis.com' && ['/youtube/v3/channels','/youtube/v3/videos'].includes(parsed.pathname)) || (parsed.origin === 'https://youtubeanalytics.googleapis.com' && parsed.pathname === '/v2/reports') || (parsed.origin === 'https://www.googleapis.com' && parsed.pathname === '/upload/youtube/v3/videos');
    ...
    const uploadEndpoint=parsed.origin==='https://www.googleapis.com'&&parsed.pathname==='/upload/youtube/v3/videos';
    const resumablePut=uploadEndpoint&&init.method==='PUT';
    const phase=thumbnailEndpoint?'thumbnail-set':resumablePut?(init.headers?.['Content-Range']?.startsWith('bytes */')?'resumable-status':'resumable-chunk'):uploadEndpoint?'resumable-create':parsed.pathname==='/youtube/v3/channels'?'channel-readback':parsed.pathname==='/youtube/v3/videos'?'processing-readback':parsed.pathname==='/v2/reports'?'analytics-read':'oauth-token';
    try {
      // A resumable 308 is a progress receipt, even when Google includes Location.
      // Expose that response without following Location or forwarding credentials.
      const result = await transport(url, { ...init, redirect: resumablePut ? 'manual' : 'error', signal: combined }); checkCancelled(signal);
      if (!result || !Number.isInteger(result.status) || result.redirected || (result.status >= 300 && result.status < 400 && !(resumablePut && result.status === 308))) throw youtubeError('YOUTUBE_UNSAFE_REDIRECT', 'Google API redirects are not permitted.');
      return result;
    } catch (error) {
      if (error instanceof YouTubeError) throw error;
      checkCancelled(signal);
      if (timeout.aborted) throw youtubeError('YOUTUBE_REQUEST_TIMEOUT', `Google request timed out during ${phase}. An interrupted upload can be resumed.`, {phase,cause:'REQUEST_TIMEOUT'});
      const cause=safeNetworkCause(error);
      throw youtubeError('YOUTUBE_NETWORK_FAILED', `Google transport failed during ${phase} (${cause}). The existing upload session is retained.`, {phase,cause});
    }
  }
  ...
  const api = {
    ...
    async beginAuthorization({ features = ['upload'] } = {}) {
      ...
      Object.entries({ client_id: clientId, redirect_uri: redirectUri, response_type: 'code', scope: requestedScopes.join(' '), state: nonce, code_challenge: crypto.createHash('sha256').update(verifier).digest('base64url'), code_challenge_method: 'S256', access_type: 'offline', prompt: 'consent select_account' }).forEach(([key, value]) => url.searchParams.set(key, value));
    ...
    async setThumbnail(uploadId,{thumbnail,filePath,signal}={}) {
      configured();checkCancelled(signal);
      if(busy||pendingAuthorization||refreshing)throw youtubeError('YOUTUBE_BUSY','Finish the active YouTube action before uploading a thumbnail.');
      busy=true;
      try{
        const stored=await state(),upload=Object.hasOwn(stored.uploads,uploadId)?stored.uploads[uploadId]:null;
        if(!upload||upload.state!=='uploaded_private'||!VIDEO_ID.test(upload.videoId||''))throw youtubeError('YOUTUBE_UPLOAD_NOT_FOUND','Choose a retained completed private upload before setting its thumbnail.');
        let prepared;
        try{prepared=await prepareYouTubeThumbnail({thumbnail,filePath,uploadReceipt:receipt(upload)});}
        catch(error){const known=new Set(['THUMBNAIL_VIDEO_PENDING','THUMBNAIL_VIDEO_MISMATCH','THUMBNAIL_INVALID','THUMBNAIL_SIZE','THUMBNAIL_CHANGED']);if(known.has(error?.code))throw youtubeError(error.code,error.message);throw youtubeError('THUMBNAIL_INVALID','The selected thumbnail could not be read.');}
        const verified=await api.verifyUploadedVideo(uploadId,{signal});checkCancelled(signal);
        const matches=current=>current?.state==='uploaded_private'&&current.videoId===upload.videoId&&current.channelId===upload.channelId&&current.sha256===upload.sha256;
        if(!matches(verified)||verified.privacyStatus!=='private')throw youtubeError('THUMBNAIL_VIDEO_MISMATCH','The retained video changed while its thumbnail was being checked.');
        const current=(await state()).uploads[uploadId];if(!matches(current))throw youtubeError('THUMBNAIL_VIDEO_MISMATCH','The retained video changed. Choose its current reviewed thumbnail.');
        const result=await authorized(prepared.url,{method:prepared.method,headers:prepared.headers,body:prepared.body},[YOUTUBE_SCOPES.upload],signal);
        if(result.status<200||result.status>=300){const failure=thumbnailUploadFailure(result.status);throw youtubeError(failure.code,failure.message,{httpStatus:result.status});}
        const resource=await json(result);if(resource.kind!=='youtube#thumbnailSetResponse'||!Array.isArray(resource.items)||!resource.items.length)throw youtubeError('THUMBNAIL_INVALID_RESPONSE','YouTube did not return a thumbnail receipt. The video upload is retained.');
        const savedThumbnail={status:'uploaded',...prepared.binding,uploadedAt:new Date(clock()).toISOString()};
        return await update(data=>{const latest=data.uploads[uploadId];if(!matches(latest))throw youtubeError('THUMBNAIL_VIDEO_MISMATCH','The retained upload changed before the thumbnail receipt could be saved.');latest.thumbnail=savedThumbnail;latest.updatedAt=new Date(clock()).toISOString();return receipt(latest);});
      }finally{busy=false;}
    },
    async saveConnectedChannel(channel) {
      if(channel!==null&&(!CHANNEL_ID.test(channel?.id||'')||typeof channel.title!=='string'))throw youtubeError('YOUTUBE_INVALID_CHANNEL','A verified Google channel is required.');
      await update(data=>{if(channel)data.connectedChannel=channel;else delete data.connectedChannel;});
    },
    async verifyUploadedVideo(uploadId,{signal}={}) {
      const stored=await state(),upload=Object.hasOwn(stored.uploads,uploadId)?stored.uploads[uploadId]:null;
      if(!upload||upload.state!=='uploaded_private'||!VIDEO_ID.test(upload.videoId||''))throw youtubeError('YOUTUBE_UPLOAD_NOT_FOUND','Finish a retained private upload before checking its processing status.');
      try{
      await verifyChannel(upload.channelId,signal);
      const result=await authorized(`https://www.googleapis.com/youtube/v3/videos?part=snippet,status,processingDetails&id=${encodeURIComponent(upload.videoId)}`,{method:'GET'},[YOUTUBE_SCOPES.readonly],signal);
      if(result.status!==200)checkHttp(result);
      const resource=await json(result),video=resource.items?.find(item=>item.id===upload.videoId);
      if(!video||video.snippet?.channelId!==upload.channelId)throw youtubeError('YOUTUBE_UPLOAD_CHANNEL_MISMATCH','The uploaded video could not be verified on the authorized channel.');
      if(video.status?.privacyStatus!=='private')throw youtubeError('YOUTUBE_UPLOAD_PRIVACY_CHANGED','The uploaded video is no longer private. No visibility change was made by VYREALM.');
      const processing=video.processingDetails?.processingStatus,uploadStatus=video.status?.uploadStatus;
      const status=processing==='succeeded'&&uploadStatus==='processed'?'succeeded':['failed','terminated'].includes(processing)||['failed','rejected','deleted'].includes(uploadStatus)?'failed':'pending';
      upload.processing={status,checkedAt:new Date(clock()).toISOString(),source:'youtube-data-api',processingStatus:processing??null,uploadStatus:uploadStatus??null};
      return await saveUpload(upload);
      }catch(error){
        const known=new Set(['YOUTUBE_UPLOAD_CHANNEL_MISMATCH','YOUTUBE_UPLOAD_PRIVACY_CHANGED','YOUTUBE_CHANNEL_MISMATCH','YOUTUBE_REAUTH_REQUIRED','YOUTUBE_NETWORK_FAILED','YOUTUBE_REQUEST_TIMEOUT','YOUTUBE_SCOPE_REQUIRED','YOUTUBE_API_REJECTED','YOUTUBE_NOT_CONNECTED','YOUTUBE_CANCELLED','YOUTUBE_INVALID_RESPONSE']);
        upload.processing={status:'unverified',checkedAt:new Date(clock()).toISOString(),source:'youtube-data-api',code:known.has(error?.code)?error.code:'YOUTUBE_VERIFICATION_FAILED'};
        await saveUpload(upload);throw error;
      }
    },
    async channels() {
    ...
    async videoAnalytics({startDate,endDate,signal}={}){
      const start=dateValue(startDate),end=dateValue(endDate);
      if(!Number.isFinite(start)||!Number.isFinite(end)||end<start||end-start>366*86400000)throw youtubeError('YOUTUBE_INVALID_ANALYTICS_QUERY','Use valid dates covering at most 366 days.');
      const url=new URL('https://youtubeanalytics.googleapis.com/v2/reports');
      Object.entries({ids:'channel==MINE',startDate,endDate,metrics:'views,estimatedMinutesWatched,averageViewDuration,likes,comments,shares',dimensions:'video',sort:'-views',maxResults:'100'}).forEach(([key,value])=>url.searchParams.set(key,value));
      const result=await authorized(url.href,{method:'GET'},[YOUTUBE_SCOPES.analytics,YOUTUBE_SCOPES.readonly],signal);if(result.status!==200)checkHttp(result);
      const report=await json(result);if(!Array.isArray(report.columnHeaders)||(report.rows!==undefined&&!Array.isArray(report.rows)))throw youtubeError('YOUTUBE_INVALID_RESPONSE','Google returned an invalid video analytics report.');
      return {status:'available',source:'youtube-analytics-api',fetchedAt:new Date(clock()).toISOString(),startDate,endDate,columnHeaders:report.columnHeaders,rows:(report.rows??[]).slice(0,100),dimension:'video',diagnostic:'At most 100 videos ranked by observed views. No impressions, CTR or retention curve is returned by this report.'};
    },
    async analytics({ startDate, endDate, videoId,signal } = {}) {
      ...
      const result = await authorized(url.href, { method: 'GET' }, [YOUTUBE_SCOPES.analytics, YOUTUBE_SCOPES.readonly],signal);
      if (result.status !== 200) checkHttp(result);
