...
export async function createYouTubeService({ dataDir, mediaDir, db, clientFactory = createYouTubeClient, vaultFactory = createYouTubeVault } = {}) {
  ...
  let clientId = ''; let configurationError = null;
  try { const file = await fs.lstat(configurationPath); if (!file.isFile() || file.isSymbolicLink() || file.size > 10000) throw new Error(); const cfg = parse(await fs.readFile(configurationPath, 'utf8')); if (!CLIENT_ID.test(cfg.clientId || '')) throw new Error(); clientId = cfg.clientId; }
  catch (error) { if (error.code !== 'ENOENT') configurationError = 'Stored YouTube Desktop client configuration is invalid. Import valid Desktop credentials to repair it.'; }
  let client = clientFactory({ clientId, vault }); let authorization = null; let auth = { phase: 'idle' }; let active = null; let shuttingDown = false; let channelWrites = Promise.resolve();
  ...
  async function publicStatus() {
    ...
    return { ...status, ...(configurationError ? { code: 'YOUTUBE_CONFIGURATION_INVALID', diagnostic: configurationError } : {}), profile: 'local-user', clientId: clientId || null, connectedChannel, auth: { ...auth }, uploadJob: active?.id || null, credentialsBundled: false };
  }
  async function saveChannel(channel) {const operation=channelWrites.then(async()=>{const stored=await vault.read();if(channel)stored.connectedChannel=channel;else delete stored.connectedChannel;await vault.write(stored);});channelWrites=operation.catch(()=>{});return operation;}
  async function configure(credentials) {
    ...
    try { await fs.writeFile(temp, JSON.stringify({ schemaVersion: 1, clientId: installed.client_id, type: 'desktop' }), { flag: 'wx', mode: 0o600 }); await fs.rename(temp, configurationPath); }
    finally { await fs.unlink(temp).catch(error => { if (error.code !== 'ENOENT') throw error; }); }
    clientId = installed.client_id; client = next; configurationError = null; return { ...result, profile: 'local-user' };
  }
  async function authorize(input) {
    only(input, ['features']); if ((await client.status()).connected || authorization || active) throw youtubeError('YOUTUBE_DISCONNECT_REQUIRED', 'Disconnect the current account before selecting another account.');
  ...
  async function enqueue(input) {
    only(input, ['projectId', 'expectedRevision', 'expectedOutputHash', 'expectedChannelId', 'confirmed', 'containsSyntheticMedia', 'metadata']);
    ...
    const owned = await ownedOutput(input); const id = crypto.randomUUID(), time = now();
  ...
  async function handle({pathname,method,body={},searchParams=new URLSearchParams()}) {
    try {
      ...
      if(pathname==='/api/youtube/disconnect'&&method==='POST'){only(body,[]);if(active)throw youtubeError('YOUTUBE_UPLOAD_BUSY','Pause the active upload before disconnecting.');const flow=authorization;authorization=null;flow?.cancel();await channelWrites;await client.disconnect();await saveChannel(null);auth={phase:'idle'};return {status:200,body:await publicStatus()};}
  ...
  return {handle,runJob,cancel,retry,async close(){shuttingDown=true;const flow=authorization;authorization=null;flow?.cancel();active?.controller.abort();await channelWrites;vault.close?.();}};
}
