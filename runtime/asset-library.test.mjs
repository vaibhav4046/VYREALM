...
test('real browser previews source media, closes with Escape, keeps filters, and resets scope when project changes',async t=>{
 // Generate a tiny moving fixture independently of retained demos or developer work folders.
 const {stdout:source}=await promisify(execFile)(fileURLToPath(new URL('../workers/tools/ffmpeg.exe',import.meta.url)),['-hide_banner','-v','error','-f','lavfi','-i','testsrc2=size=160x90:rate=12:duration=1','-c:v','libvpx','-deadline','realtime','-cpu-used','8','-threads','1','-an','-f','webm','pipe:1'],{windowsHide:true,timeout:20000,encoding:'buffer',maxBuffer:1024*1024});
 ...
 let browser;t.after(async()=>{server.closeAllConnections();const closed=new Promise(r=>server.close(r));try{await browser?.close();}finally{await closed;}});
