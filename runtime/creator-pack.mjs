...
function hashtags(title,brief,script){
 ...
 const scored=new Map();for(const [content,weight]of [[title,4],[brief,2],[script,1]])for(const word of content.replace(/\b(?:https?:\/\/|www\.)[^\s<>]+/gi,' ').normalize('NFKD').replace(/\p{M}/gu,'').toLowerCase().match(/[a-z][a-z0-9]{2,23}/g)||[]){if(stop.has(word))continue;scored.set(word,(scored.get(word)||0)+weight);}
