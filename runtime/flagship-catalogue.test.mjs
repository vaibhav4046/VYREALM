...
test('catalogue UI selects, plays, downloads and withdraws a verified film through the real API', { timeout: 90_000 }, async t => {
  ...
  try {
    ...
    assert.deepEqual(errors, [], 'The app must load before the catalogue journey');
    await page.locator('#chatProject').selectOption({ label: 'Gate fixture' });
    mark('select flagship');
    await page.locator('.sidebar [data-nav="Catalog"]').click();
    await page.getByRole('button', { name: 'Add to catalogue', exact: true }).click();
    await page.locator('.catalog-film').waitFor();
    mark('play video');
    ...
    await page.locator('.film-evidence summary').click();
    await page.getByRole('button', { name: 'Remove from catalogue' }).click();
    await page.getByText('No films selected', { exact: true }).waitFor();
    assert.equal(await page.locator('.catalog-film').count(), 0);
