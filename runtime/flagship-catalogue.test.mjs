...
test('catalogue UI selects, plays, downloads and withdraws a verified film through the real API', { timeout: 90_000 }, async t => {
  ...
  try {
    ...
    await page.locator('.project-card').filter({ hasText: 'Gate fixture' }).getByRole('button', { name: 'Continue in Studio chat ↗' }).click();
    await page.locator('.sidebar [data-nav="Catalog"]').click();
    ...
    await page.getByRole('button', { name: 'Continue in Studio chat ↗' }).waitFor();
