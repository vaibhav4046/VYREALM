...
export function createLocalAutomations({ db, dispatchRender, prepareCreatorPack, cancelJob, verifyOutput, clock = () => Date.now() } = {}) {
  ...
  async function requestStop(automation, target, diagnostic) {
    ...
    const output = { ...parse(automation.output), afterCancel:target };
    save(automation.id, { status:'cancelling', output:JSON.stringify(output), diagnostic:JSON.stringify(diagnostic) });
    if (!currentJob || !pending.has(currentJob.status)) return finish(automation.id,target,diagnostic);
    if (currentJob.status === 'queued') {
      db.prepare("UPDATE jobs SET status='cancelled',stage='Local automation cancelled before dispatch',updated_at=? WHERE id=? AND status='queued'").run(time(),currentJob.id);
    } else {
      try { await cancelJob(currentJob.id); }
      catch (error) { save(automation.id, { diagnostic:JSON.stringify({ ...diagnostic, cancellation:boundedError(error), message:`${diagnostic.message} Waiting for the owned job to stop; its worker has not confirmed cancellation.` }) }); }
    }
    if (!pending.has(job(currentJob.id)?.status)) return finish(automation.id,target,diagnostic);
  ...
  return {
    create, get:id => view(row(id)), list:() => db.prepare('SELECT * FROM local_automations ORDER BY created_at DESC,id').all().map(view), tick,
    recover:tick, cancel,
    async close() { if (closed) return; closed = true; await flight; db.prepare('DELETE FROM local_automation_scheduler WHERE owner_id=?').run(owner); coordinators.delete(db); },
  };
}
