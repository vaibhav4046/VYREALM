...
test('one automation dispatches at a time until the active workflow reaches review',async t=>{
  const f=await fixture(t), a=f.create(), b=f.create();await f.scheduler.tick();await f.scheduler.tick();assert.equal(f.calls.dispatch.length,1);
  ...
  await f.scheduler.tick();const other=[a,b].find(item=>item.id!==active.id);assert.equal(f.scheduler.get(other.id).status,'paused');assert.equal(f.calls.dispatch.length,1);
});
