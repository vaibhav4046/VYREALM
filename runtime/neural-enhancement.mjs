...
export async function enhanceNeuralClip({input,receiptPath,output,runtimeDir=process.env.VYRELUM_RUNTIME_DIR||resolve('data/runtime'),onProgress=()=>{}}){
  ...
  if(receipt.provenance?.generationStatus!=='generated'||!receipt.provenance?.evidenceHash||receipt.provenance?.semanticQuality?.startsWith('rejected'))throw new Error('VERIFIED_NEURAL_SOURCE_REQUIRED');
  if(hash(await readFile(source))!==receipt.provenance.outputHash)throw new Error('SOURCE_PROVENANCE_HASH_MISMATCH');
  ...
  const expectedStorage=Number(receipt.provenance.resolution.width)*Number(receipt.provenance.resolution.height)*16*3*Number(receipt.provenance.frameCount)*2+2*1024**3;
  ...
  const started=Date.now(),fps=Number(receipt.provenance.fps),frameCount=Number(receipt.provenance.frameCount),duration=Number(receipt.durationSeconds);
