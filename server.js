...
async function stopEngine(){
  if(stopping)return;stopping=true;clearInterval(automationTimer);await localAutomations?.close();
...
async function runJob(id){
 ...
 if((path==='/api/automations'||automationCancel)&&req.method==='POST'){
   ...
   }catch(error){return send(res,error.code==='AUTOMATION_NOT_FOUND'?404:/REVISION|CONFLICT|SCHEDULER_ACTIVE/.test(error.code||'')?409:400,{code:error.code||'AUTOMATION_FAILED',error:error.message});}
 }
 ...
 const publicFiles=new Set(['/','/index.html','/app.js','/studio-chat.js','/cinematic-studio.js','/timeline-editor.js','/asset-library.js','/asset-library.css','/mcp-connection.js','/creator-templates.js','/creator-templates.css','/local-automations.js','/local-automations.css','/youtube-settings.js','/styles.css']);
...
for(const pending of db.prepare("SELECT id,input FROM jobs WHERE status='queued' ORDER BY created_at").all())if(!json(Buffer.from(pending.input||'{}')).automationId)void runJob(pending.id);
