...
async function runJob(id){
      ...
      if(val?.validated!==false&&val?.status!=='blocked'){
        ...
        if(r.project_id&&canPromote){
          ...
          if(pr){
            ...
            else if(r.type==='sound-design'&&created.audio){d.audioTracks=[...(d.audioTracks||[]),{id:randomUUID(),assetId:created.audio,name:'Rain & tension',start:0,gain:0.7,muted:false,provenance:val.provenance}];d.latestSoundDesign={assetId:created.audio,jobId:id,evidence:val.evidence,provenance:val.provenance};}
 ...
 if(path==='/api/audio/sound-design'&&req.method==='POST'){
   ...
   const id=randomUUID(),t=now(),input={projectId:project.id,revision:project.revision,durationSeconds,seed:Number.isSafeInteger(x.seed)?x.seed:713,footsteps,shelterAt:x.shelterAt,climaxAt:x.climaxAt};
 ...
 if(path==='/api/generation/enhance'&&req.method==='POST'){
   ...
   const id=randomUUID(),t=now(),input={type:'enhance',projectId:project.id,revision:project.revision,sourceAssetId,sourcePath:join(jobsDir,sourceJob.id,receipt.provenance.outputPath),receiptPath:join(jobsDir,sourceJob.id,'result.json')};
