import { receiveMediaUpload } from './runtime/media-upload.mjs';
import { assertNoActiveNeuralJobs } from './runtime/heavy-work-admission.mjs';
import { createHeavyAdmissionGate, heavyAdmissionKind } from './runtime/heavy-http-admission.mjs';
import { createOriginalGenerationService } from './runtime/original-generation-service.mjs';
import { createAudioOnboarding } from './runtime/audio-onboarding.mjs';
...
import { projectTranscriptionUpdate } from './runtime/project-transcription.mjs';
import { createYouTubeInsights } from './publishing/youtube-insights.mjs';
import { createInsightProject } from './runtime/insight-project.mjs';
import { createThumbnailService } from './runtime/thumbnail-service.mjs';
...
import { TOOL_DEFINITIONS } from './runtime/automation-tools.mjs';
import { mcpConnectionConfig, checkMcpConnection } from './runtime/mcp-connection.mjs';
import { createLocalAutomations, createLocalAutomationVerifier, localAutomationJobId } from './runtime/local-automations.mjs';
import { createLocalAutomationBatches } from './runtime/local-automation-batch.mjs';
...
let mcpCheckFlight=null;
let audioOnboardingFlight=null;
async function audioOnboardingService(){audioOnboardingFlight??=createAudioOnboarding({root,runtimeDir:process.env.VYRELUM_RUNTIME_DIR||join(dataDir,'runtime')});try{return await audioOnboardingFlight;}catch(error){audioOnboardingFlight=null;throw error;}}
let localAutomations=null,automationTimer=null,stopping=false;
const providerRegistry=createProviderRegistry();
const youtubeService=await createYouTubeService({dataDir,mediaDir,db,publisherConfigPath:process.env.VYRELUM_YOUTUBE_DESKTOP_CONFIG});
const youtubeInsights=createYouTubeInsights({db,readConnection:youtubeService.readConnection,fetchDailyAnalytics:youtubeService.fetchDailyAnalytics,fetchVideoAnalytics:youtubeService.fetchVideoAnalytics});
const thumbnailService=createThumbnailService({db,jobsDir,root});
let youtubeInsightsTimer=null;
...
const originalGenerationService=await createOriginalGenerationService({db,dataDir,jobsDir,mediaDir,root});
...
async function prepareCreatorPack(projectId,input,internal={}){
 ...
 if(internal.automationId){const automation=db.prepare('SELECT * FROM local_automations WHERE id=?').get(internal.automationId);if(!automation||automation.status!=='running'||automation.step!=='creator-pack'||automation.project_id!==projectId||automation.job_id!==internal.jobId||internal.jobId!==localAutomationJobId(automation.id,'creator-pack')||json(Buffer.from(automation.output)).postRenderRevision!==row.revision)throw Object.assign(Error('Creator-pack automation ownership changed.'),{code:'AUTOMATION_JOB_OWNERSHIP'});}
 const id=internal.jobId||randomUUID(),time=now(),jobRoot=join(jobsDir,id);
 db.prepare('INSERT INTO jobs (id,project_id,revision,type,status,progress,stage,input,attempts,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(id,projectId,row.revision,'creator-pack','running',0,'Preparing thumbnail and draft creator materials',JSON.stringify({sourceAssetId:sourceId,progressUnit:'percent',...(internal.automationId?{automationId:internal.automationId}:{})}),1,time,time);
 creatorPackRunning=true;
 try{
...
async function stopEngine(){
  if(stopping)return;stopping=true;clearInterval(automationTimer);clearInterval(youtubeInsightsTimer);await youtubeInsights.close();await localAutomations?.close();
  if(audioOnboardingFlight)await (await audioOnboardingFlight).close();
  await originalGenerationService.close();
...
for(const interrupted of db.prepare("SELECT * FROM jobs WHERE status IN ('running','staging','validating','cancelling')").all())if(!originalGenerationService.owns(interrupted))db.prepare("UPDATE jobs SET status='failed',stage='interrupted',error='Control plane restarted before worker completion',updated_at=? WHERE id=?").run(now(),interrupted.id);
...
const pdoc=r=>({...json(Buffer.from(r.document)),id:r.id,revision:r.revision,createdAt:r.created_at,updatedAt:r.updated_at});const adoc=r=>({...json(Buffer.from(r.document)),id:r.id,projectId:r.project_id,url:`/media/${r.id}`});const jdoc=r=>{const input={...json(Buffer.from(r.input||'{}'))};delete input.automationId;delete input.projectSnapshot;delete input.ownerPid;return {...input,...(originalGenerationService.owns(r)?{progressUnit:'percent'}:{}),id:r.id,projectId:r.project_id,revision:r.revision,type:r.type,status:r.status,progress:r.progress,stage:r.stage,output:r.output?json(Buffer.from(r.output)):null,error:r.error,attempts:r.attempts,createdAt:r.created_at,updatedAt:r.updated_at};};
async function cancelLocalAutomationJob(id){
 const row=db.prepare('SELECT * FROM jobs WHERE id=?').get(id),input=row&&json(Buffer.from(row.input||'{}'));
 if(!row||!input.automationId||row.id!==localAutomationJobId(input.automationId,row.type))throw Object.assign(Error('This job is not owned by a local automation.'),{code:'AUTOMATION_JOB_OWNERSHIP'});
 if(row.type==='creator-pack')throw Object.assign(Error('Creator materials finish their current CPU operation; the automation will then stop with files retained.'),{code:'CREATOR_PACK_CONTROL'});
 if(row.type!=='render')throw Object.assign(Error('Unsupported automation worker.'),{code:'AUTOMATION_JOB_OWNERSHIP'});
 if(['queued','running','staging','validating','cancelling'].includes(row.status)){cancelling.add(id);db.prepare("UPDATE jobs SET status='cancelling',stage='Automation cancellation requested',updated_at=? WHERE id=?").run(now(),id);await terminateProcessTree(children.get(id));if(!children.has(id)){cancelling.delete(id);db.prepare("UPDATE jobs SET status='cancelled',stage='Automation cancelled; completed files retained',updated_at=? WHERE id=?").run(now(),id);}}
}
function automationRenderSnapshot(job,input){
 if(!Object.hasOwn(input,'automationId')&&!Object.hasOwn(input,'projectSnapshot'))return null;
 const automation=typeof input.automationId==='string'&&db.prepare('SELECT * FROM local_automations WHERE id=?').get(input.automationId);
 if(!automation||job.type!=='render'||automation.status!=='running'||automation.step!=='render'||automation.job_id!==job.id||automation.project_id!==job.project_id||automation.expected_revision!==job.revision||job.id!==localAutomationJobId(automation.id,'render')||JSON.stringify(input.projectSnapshot)!==JSON.stringify(json(Buffer.from(automation.snapshot))))throw Object.assign(Error('The render job does not match its saved local automation.'),{code:'AUTOMATION_JOB_OWNERSHIP'});
 if(db.prepare('SELECT revision FROM projects WHERE id=?').get(job.project_id)?.revision!==automation.expected_revision)throw Object.assign(Error('The project changed while this scheduled render waited. The automation is paused; newer edits are retained.'),{code:'AUTOMATION_PROJECT_CHANGED'});
 return json(Buffer.from(automation.snapshot));
}
localAutomations=createLocalAutomations({db,dispatchRender:id=>runJob(id),prepareCreatorPack,cancelJob:cancelLocalAutomationJob,verifyOutput:createLocalAutomationVerifier({ffmpeg:process.env.VYRELUM_FFMPEG||join(root,'workers/tools',process.platform==='win32'?'ffmpeg.exe':'ffmpeg'),ffprobe:process.env.VYRELUM_FFPROBE||join(root,'workers/tools',process.platform==='win32'?'ffprobe.exe':'ffprobe')})});
const localAutomationBatches=createLocalAutomationBatches({db,automations:localAutomations});
...
async function runJob(id){
  if(originalGenerationService.owns(db.prepare('SELECT * FROM jobs WHERE id=?').get(id)))return;
  ...
  const raw=json(Buffer.from(r.input||'{}')); let inputData=raw,automationSnapshot=null;
  try{automationSnapshot=automationRenderSnapshot(r,raw);}catch(error){db.prepare("UPDATE jobs SET status='blocked',stage='Scheduled render paused',error=?,updated_at=? WHERE id=?").run(`${error.code}: ${error.message}`,now(),id);gpuLease=null;await releaseFileLease();return;}
  ...
  if((r.type==='scene'||r.type==='render')){const pr=r.project_id&&db.prepare('SELECT * FROM projects WHERE id=?').get(r.project_id);const d=automationSnapshot||(pr?json(Buffer.from(pr.document)):{});
  ...
  if(inputData.kind==='timeline'){
    try{
      ...
      const d=automationSnapshot||json(Buffer.from(db.prepare('SELECT document FROM projects WHERE id=?').get(r.project_id).document));
  ...
  if(automationSnapshot)try{automationRenderSnapshot(r,raw);}catch(error){db.prepare("UPDATE jobs SET status='blocked',stage='Scheduled render paused',error=?,updated_at=? WHERE id=?").run(`${error.code}: ${error.message}`,now(),id);gpuLease=null;await releaseFileLease();return;}
  ...
  child.on('exit',async code=>{
    ...
    if(code===0&&existsSync(result)){
      ...
      if(val?.validated!==false&&val?.status!=='blocked'){
        ...
        let canPromote=Boolean(currentJob&&currentJob.status!=='cancelled'&&currentJob.status!=='cancelling'&&(!currentProject||Number(currentProject.revision)===Number(r.revision||currentProject.revision)));
        ...
        const retainCreatedAssets=()=>{for(const aid of Object.values(created)){const asset=db.prepare('SELECT document FROM assets WHERE id=?').get(aid);if(asset)db.prepare('UPDATE assets SET project_id=NULL,document=? WHERE id=?').run(JSON.stringify({...json(Buffer.from(asset.document)),projectRevisionMatch:false}),aid);}};
        // File copies above yield to edits/cancellation. Recheck their exact
        // ownership immediately before promotion, retaining stale artifacts.
        const promotionJob=db.prepare('SELECT status,revision,project_id FROM jobs WHERE id=?').get(id);
        const promotionProject=r.project_id?db.prepare('SELECT revision FROM projects WHERE id=?').get(r.project_id):null;
        const cancelledDuringCopy=cancelling.has(id)||['cancelled','cancelling'].includes(promotionJob?.status);
        canPromote=Boolean(canPromote&&promotionJob&&!cancelledDuringCopy&&(!r.project_id||promotionProject&&Number(promotionProject.revision)===Number(r.revision||promotionProject.revision)));
        if(!canPromote)retainCreatedAssets();
        if(cancelledDuringCopy){cancelling.delete(id);db.prepare("UPDATE jobs SET status='cancelled',stage='cancelled; output retained',error=NULL,output=?,updated_at=? WHERE id=?").run(JSON.stringify({...val,assets:created,promoted:false,diagnostics:['CANCELLED_OUTPUT_RETAINED']}),now(),id);return;}
        if(r.project_id&&canPromote){
          ...
          if(pr){
            ...
            else if(r.type==='transcribe'){
              try{Object.assign(d,projectTranscriptionUpdate({project:d,sourceAssetId:raw.assetId,jobId:id,segments:val.segments,provenance:val.provenance}));}
              catch(error){retainCreatedAssets();db.prepare("UPDATE jobs SET status='blocked',stage='Caption placement needs attention',error=?,output=?,updated_at=? WHERE id=?").run(error.message,JSON.stringify({...val,assets:created,promoted:false,diagnostics:[error.code||'INVALID_CAPTION_PLACEMENT']}),now(),id);return;}
            }
            ...
            db.exec('BEGIN IMMEDIATE');try{const changed=db.prepare('UPDATE projects SET revision=?,document=?,updated_at=? WHERE id=? AND revision=?').run(nextRevision,nextDocument,updated,r.project_id,pr.revision);if(changed.changes!==1){db.exec('ROLLBACK');canPromote=false;retainCreatedAssets();}else{db.prepare('INSERT INTO project_revisions VALUES (?,?,?,?)').run(r.project_id,nextRevision,nextDocument,updated);db.exec('COMMIT');}}catch(error){try{db.exec('ROLLBACK');}catch{}throw error;}
          }
        }
 ...
 const automationCancel=path.match(/^\/api\/automations\/([a-zA-Z0-9_-]+)\/cancel$/);
 if(path==='/api/automations'&&req.method==='GET')return send(res,200,localAutomations.list());
 if(path==='/api/automations/summary'&&req.method==='GET')return send(res,200,localAutomations.summary());
 const automationBatchId=path.match(/^\/api\/automation-batches\/([a-zA-Z0-9_-]+)$/);
 if(path==='/api/automation-batches'||automationBatchId){
   try{
     if(req.method==='GET'){
       if(automationBatchId){const batch=localAutomationBatches.get(automationBatchId[1]);return send(res,batch?200:404,batch||{code:'AUTOMATION_BATCH_NOT_FOUND',error:'This saved-edit batch was not found.'});}
       const query=new URL(req.url,'http://localhost').searchParams;
       if([...query.keys()].some(k=>!['limit','before'].includes(k)))return send(res,400,{code:'AUTOMATION_BATCH_REQUEST',error:'Only limit and before are supported.'});
       return send(res,200,localAutomationBatches.list({...(query.has('limit')?{limit:Number(query.get('limit'))}:{}),...(query.has('before')?{before:query.get('before')}:{})}));
     }
     if(req.method==='POST'&&!automationBatchId){
       const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>32768)return send(res,413,{code:'AUTOMATION_BATCH_TOO_LARGE',error:'Saved-edit batch requests are limited to 32 KiB.'});chunks.push(chunk);}
       let input;try{input=JSON.parse(Buffer.concat(chunks).toString('utf8'));}catch{return send(res,400,{code:'AUTOMATION_BATCH_REQUEST',error:'A valid JSON object is required.'});}
       const batch=localAutomationBatches.create(input);send(res,batch.admissionStatus==='complete'?201:202,batch);void localAutomations.tick().catch(error=>console.error('Local automation:',error.message));return;
     }
     return send(res,405,{code:'AUTOMATION_BATCH_METHOD',error:'Use GET to inspect a batch or POST to create one.'});
   }catch(error){return send(res,/CONFLICT|EVIDENCE/.test(error.code||'')?409:400,{code:error.code||'AUTOMATION_BATCH_FAILED',error:error.message});}
 }
 if((path==='/api/automations'||automationCancel)&&req.method==='POST'){
   let body;try{const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>8192)return send(res,413,{code:'AUTOMATION_REQUEST_TOO_LARGE',error:'Local automation requests are limited to 8 KiB.'});chunks.push(chunk);}body=JSON.parse(Buffer.concat(chunks).toString('utf8')||'{}');}catch{return send(res,400,{code:'AUTOMATION_REQUEST',error:'A valid JSON object is required.'});}
   try{
     if(automationCancel){if(!body||typeof body!=='object'||Array.isArray(body)||Object.keys(body).length)return send(res,400,{code:'AUTOMATION_REQUEST',error:'Cancellation accepts an empty JSON object.'});return send(res,200,await localAutomations.cancel(automationCancel[1]));}
     const automation=localAutomations.create(body);send(res,201,automation);void localAutomations.tick().catch(error=>console.error('Local automation:',error.message));return;
   }catch(error){return send(res,error.code==='AUTOMATION_NOT_FOUND'?404:error.code==='AUTOMATION_QUEUE_FULL'?429:/REVISION|CONFLICT|SCHEDULER_ACTIVE/.test(error.code||'')?409:400,{code:error.code||'AUTOMATION_FAILED',error:error.message});}
 }


 const originalProject=path.match(/^\/api\/projects\/([a-zA-Z0-9-]+)\/original-generation$/);
 const originalAction=path.match(/^\/api\/original-generation\/([a-zA-Z0-9-]+)\/(review|animate|retry|cancel)$/);
 if(originalProject&&req.method==='GET'){try{return send(res,200,originalGenerationService.state(originalProject[1]));}catch(error){return send(res,error.code==='ORIGINAL_PROJECT_NOT_FOUND'?404:400,{code:error.code||'ORIGINAL_FAILED',error:error.message});}}
 if((originalProject||originalAction)&&req.method==='POST'){
   let body;try{const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>32768)return send(res,413,{code:'ORIGINAL_REQUEST_TOO_LARGE',error:'Original shot requests are limited to 32 KiB.'});chunks.push(chunk);}body=JSON.parse(Buffer.concat(chunks).toString('utf8')||'{}');}catch{return send(res,400,{code:'ORIGINAL_REQUEST',error:'A valid JSON object is required.'});}
   if(!body||typeof body!=='object'||Array.isArray(body))return send(res,400,{code:'ORIGINAL_REQUEST',error:'A JSON object is required.'});
   try{
     if(originalProject){if(Object.hasOwn(body,'projectId'))return send(res,400,{code:'ORIGINAL_REQUEST',error:'The project is selected by this route.'});return send(res,202,await originalGenerationService.start({...body,projectId:originalProject[1]}));}
     const [,jobId,action]=originalAction;
     if(action==='cancel'){if(Object.keys(body).length)return send(res,400,{code:'ORIGINAL_REQUEST',error:'Cancellation accepts an empty object.'});return send(res,200,{job:await originalGenerationService.cancel(jobId)});}
     if(Object.hasOwn(body,'jobId')||Object.hasOwn(body,'keyframeJobId'))return send(res,400,{code:'ORIGINAL_REQUEST',error:'The owned job is selected by this route.'});
     return send(res,action==='review'?200:202,await originalGenerationService[action]({...body,[action==='animate'?'keyframeJobId':'jobId']:jobId}));
   }catch(error){return send(res,/NOT_FOUND/.test(error.code||'')?404:/BUSY|REVISION|CONFLICT|REVIEW|REJECTED|OTHER_PROCESS|RETRY/.test(error.code||'')?409:400,{code:error.code||'ORIGINAL_FAILED',error:error.message});}
 }

 if(path==='/api/audio/setup'&&req.method==='GET')return send(res,200,await(await audioOnboardingService()).status());
 const audioSetupCancel=path.match(/^\/api\/audio\/setup\/([a-zA-Z0-9-]+)\/cancel$/);
 if((path==='/api/audio/setup'||audioSetupCancel)&&req.method==='POST'){
   let body;try{const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>4096)return send(res,413,{code:'AUDIO_SETUP_REQUEST_TOO_LARGE',error:'Audio setup requests are limited to 4 KiB.'});chunks.push(chunk);}body=JSON.parse(Buffer.concat(chunks).toString('utf8')||'{}');}catch{return send(res,400,{code:'AUDIO_SETUP_REQUEST',error:'A valid JSON object is required.'});}
   if(!body||typeof body!=='object'||Array.isArray(body)||Object.keys(body).some(key=>audioSetupCancel||key!=='mode')||!audioSetupCancel&&!['install','verify'].includes(body.mode))return send(res,400,{code:'AUDIO_SETUP_REQUEST',error:'Choose install or verify; cancellation accepts an empty object.'});
   try{const service=await audioOnboardingService();if(audioSetupCancel)return send(res,200,await service.cancel(audioSetupCancel[1]));
     if(db.prepare("SELECT id FROM jobs WHERE type='runtime-setup' AND status IN ('queued','running','cancelling')").get())return send(res,409,{code:'AUDIO_SETUP_BUSY',error:'Let the current runtime setup finish before starting audio setup.'});
     return send(res,202,await service.start(body));
   }catch(error){return send(res,/BUSY|RUNNING|EXISTS|CONFIGURED/.test(error.code||'')?409:400,{code:error.code||'AUDIO_SETUP_FAILED',error:error.message});}
 }
 if(path==='/api/mcp/config'&&req.method==='GET')return send(res,200,{config:mcpConnectionConfig({root,port:Number(process.env.PORT||4173)}),tools:TOOL_DEFINITIONS.map(tool=>({name:tool.name,description:tool.description,available:!tool.description.includes('Currently unavailable'),inputSchema:tool.inputSchema}))});
 if(path==='/api/mcp/check'&&req.method==='POST'){if(mcpCheckFlight)return send(res,409,{error:'A bridge check is already running',code:'MCP_CHECK_BUSY'});try{mcpCheckFlight=checkMcpConnection({root,port:Number(process.env.PORT||4173)});return send(res,200,await mcpCheckFlight);}catch(error){return send(res,500,{error:error.message,code:'MCP_CHECK_FAILED'});}finally{mcpCheckFlight=null;}}
 const thumbnailRoute=path.match(/^\/api\/(?:projects\/([a-zA-Z0-9-]+)\/thumbnails|thumbnail\/projects\/([a-zA-Z0-9-]+))$/);
 if(thumbnailRoute&&['GET','POST'].includes(req.method)){
   try{const result=await thumbnailService.handle({pathname:`/api/thumbnail/projects/${thumbnailRoute[1]||thumbnailRoute[2]}`,method:req.method,body:req.method==='POST'?json(await readBody(req)):undefined});return send(res,req.method==='POST'?201:200,result);}
   catch(error){return send(res,/BUSY|REVISION/.test(error.code||'')?409:400,{code:error.code||'THUMBNAIL_FAILED',error:error.message});}
 }
 if(path.startsWith('/api/youtube/')){
   ...
   if(path==='/api/youtube/insights/brief'&&req.method==='POST'){
     try{return send(res,201,await createInsightProject({db,insights:youtubeInsights,input:body}));}
     catch(error){return send(res,error.status||409,{code:error.code||'INSIGHT_PROJECT_FAILED',error:error.message});}
   }
   if(path==='/api/youtube/disconnect'&&req.method==='POST')await youtubeInsights.pauseForDisconnect();
   const result=await (path.startsWith('/api/youtube/insights')?youtubeInsights:youtubeService).handle({pathname:path,method:req.method,body,searchParams:new URL(req.url,'http://localhost').searchParams});
 ...
 if(youtubeJobControl&&req.method==='POST'&&originalGenerationService.owns(db.prepare('SELECT * FROM jobs WHERE id=?').get(youtubeJobControl[1]))){try{if(youtubeJobControl[2]==='cancel')return send(res,200,await originalGenerationService.cancel(youtubeJobControl[1]));return send(res,409,{code:'ORIGINAL_OWNED_JOB',error:'Retry this shot from Production plan against its saved revision.'});}catch(error){return send(res,409,{code:error.code||'ORIGINAL_FAILED',error:error.message});}}
 if(youtubeJobControl&&req.method==='POST'){const controlled=db.prepare('SELECT input FROM jobs WHERE id=?').get(youtubeJobControl[1]);if(controlled&&json(Buffer.from(controlled.input||'{}')).automationId)return send(res,409,{code:'LOCAL_AUTOMATION_CONTROL',error:'Cancel this workflow from Automations. To rerun, create a new automation from the saved project; generic job retries cannot replace its recorded execution.'});}
 ...
 if(youtubeJobControl&&req.method==='POST'&&db.prepare('SELECT type FROM jobs WHERE id=?').get(youtubeJobControl[1])?.type==='thumbnail')return send(res,409,{code:'THUMBNAIL_CONTROL',error:'Create another thumbnail from Thumbnails after this short composition finishes. Generic video-render controls do not apply.'});
 ...
 if(path==='/api/runtime/setup'&&req.method==='POST'){
   if(audioOnboardingFlight&&(await(await audioOnboardingService()).status()).job?.status==='running')return send(res,409,{code:'AUDIO_SETUP_BUSY',error:'Let the current audio setup finish before changing the video runtime.'});
   const x=json(await readBody(req));
 ...
 if(path==='/api/projects/import'&&req.method==='POST'){
   try{
     ...
     if(Object.hasOwn(doc,'originalGeneration')){doc.importedOriginalGenerationEvidence={status:'imported-unverified',sourceMethod:'imported-project-bundle',importedAt:t,value:doc.originalGeneration};delete doc.originalGeneration;}
     if(Object.hasOwn(doc,'insightSource')){doc.importedInsightSource={status:'imported-unverified',sourceMethod:'imported-project-bundle',importedAt:t,value:doc.insightSource};delete doc.insightSource;}
     if(Object.hasOwn(doc,'thumbnailStudio')){doc.importedThumbnailStudio={status:'imported-unverified',sourceMethod:'imported-project-bundle',importedAt:t,value:doc.thumbnailStudio};delete doc.thumbnailStudio;}
 ...
 if(path==='/api/assets'&&req.method==='POST'){const id=randomUUID(),projectId=req.headers['x-project-id']||null,t=now();if(projectId&&!db.prepare('SELECT 1 FROM projects WHERE id=?').get(projectId))return send(res,404,{code:'MEDIA_PROJECT_NOT_FOUND',error:'Open a saved project before importing media.'});if(mediaUploadActive){res.setHeader('Connection','close');return send(res,409,{code:'MEDIA_UPLOAD_BUSY',error:'Another import is in progress. Let it finish before adding the next file.'});}mediaUploadActive=true;let imported;try{imported=await receiveMediaUpload({request:req,mediaDir,id});}catch(error){res.setHeader('Connection','close');res.once('finish',()=>req.destroy());return send(res,error.code==='MEDIA_UPLOAD_TOO_LARGE'?413:error.code==='MEDIA_DISK_LOW'?507:400,{code:error.code||'MEDIA_UPLOAD_FAILED',error:error.message});}finally{mediaUploadActive=false;}const {name,path:file,mime,size}=imported,doc={name,mime,size,kind:mime.split('/')[0]||'file'};db.prepare('INSERT INTO assets VALUES (?,?,?,?,?)').run(id,projectId,JSON.stringify(doc),file,t);if(projectId&&doc.kind==='image'){const pr=db.prepare('SELECT * FROM projects WHERE id=?').get(projectId);if(pr){const d=json(Buffer.from(pr.document));if(d.mode==='product'&&!d.productAssetId){const next={...d,productAssetId:id};db.prepare('UPDATE projects SET document=?,updated_at=? WHERE id=?').run(JSON.stringify(next),t,projectId);}}}return send(res,201,adoc(db.prepare('SELECT * FROM assets WHERE id=?').get(id)));}
 ...
 const publicFiles=new Set(['/','/index.html','/app.js','/studio-chat.js','/cinematic-studio.js','/timeline-editor.js','/asset-library.js','/asset-library.css','/mcp-connection.js','/creator-templates.js','/creator-templates.css','/local-automations.js','/audio-onboarding.js','/original-generation.js','/local-automations.css','/youtube-settings.js','/youtube-upload.js','/youtube-insights.js','/thumbnail-studio.js','/styles.css']);

const heavyAdmission=createHeavyAdmissionGate();
async function serveApi(req,res,path){
 const kind=heavyAdmissionKind(path,req.method);if(!kind)return api(req,res,path);
 if(!auth(req,res))return;
 try{return await heavyAdmission(async()=>{
  if(kind==='audio'||kind==='runtime')assertNoActiveNeuralJobs(db,{operation:kind==='audio'?'Voice and caption setup':'Runtime setup'});
  if(kind==='neural'||kind==='production'||kind==='runtime'){
   if(audioOnboardingFlight){const audio=await(await audioOnboardingService()).status();if(['running','cancelling'].includes(audio.job?.status))throw Object.assign(Error('Let voice and caption setup finish or cancel it before starting another heavy operation.'),{code:'HEAVY_WORK_BUSY',jobId:audio.job.id});}
   const other=db.prepare("SELECT id,type FROM jobs WHERE type IN ('raw-footage-edit','runtime-setup') AND status IN ('queued','running','staging','validating','cancelling') ORDER BY created_at LIMIT 1").get();
   if(other)throw Object.assign(Error('Let the active '+other.type+' operation finish or cancel it before starting neural or production work.'),{code:'HEAVY_WORK_BUSY',jobId:other.id});
  }
  return api(req,res,path);
 });}catch(error){if(error.code==='HEAVY_WORK_BUSY')return send(res,409,{code:error.code,error:error.message,...(error.jobId?{jobId:error.jobId}:{}),...(error.jobType?{jobType:error.jobType}:{})});throw error;}
}

 createServer(async(req,res)=>{const u=new URL(req.url,'http://localhost');if(u.pathname.startsWith('/api/'))return serveApi(req,res,u.pathname).catch(error=>send(res,500,{error:error.message,code:'LOCAL_API_FAILED'}));if(u.pathname.startsWith('/media/')){const o=req.headers.origin;if(o&&!/^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(o)){res.writeHead(403);return res.end()};const r=db.prepare('SELECT * FROM assets WHERE id=?').get(u.pathname.split('/')[2]);if(!r)return res.writeHead(404).end();return streamLocalMedia(req,res,r.path,json(Buffer.from(r.document)).mime);}if(req.method!=='GET'||!publicFiles.has(u.pathname))return res.writeHead(404).end();const p=normalize(join(root,u.pathname==='/'?'index.html':u.pathname));if(!p.startsWith(root)||!existsSync(p))return res.writeHead(404).end();res.writeHead(200,{'content-type':mime[extname(p)]||'application/octet-stream'});createReadStream(p).pipe(res)}).listen(Number(process.env.PORT||4173),'127.0.0.1',()=>console.log(`VYRELUM local control plane listening on http://localhost:${process.env.PORT||4173}`));



for(const pending of db.prepare("SELECT * FROM jobs WHERE status='queued' ORDER BY created_at").all())if(!originalGenerationService.owns(pending)&&!json(Buffer.from(pending.input||'{}')).automationId)void runJob(pending.id);
try{localAutomationBatches.recover();}catch(error){console.error('Saved-edit batch recovery:',error.message);}
void localAutomations.recover().catch(error=>console.error('Local automation recovery:',error.message));
automationTimer=setInterval(()=>void localAutomations.tick().catch(error=>console.error('Local automation:',error.message)),1000);automationTimer.unref();
void youtubeInsights.tick().catch(()=>console.error('YouTube insights startup check stopped; inspect its saved status.'));
youtubeInsightsTimer=setInterval(()=>void youtubeInsights.tick().catch(()=>console.error('YouTube insights check stopped; inspect its saved status.')),60000);youtubeInsightsTimer.unref();
