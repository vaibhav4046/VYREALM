...
export function validateClipEdit(clip, patch, sourceDuration = null, fps = 24) {
  ...
  if(next.framing!==undefined&&!['fit','crop-left','crop-right','crop-center','crop-custom'].includes(next.framing))throw Error('Choose a supported clip framing.');
  if(next.cropPosition!==undefined&&(!next.cropPosition||['x','y'].some(axis=>typeof next.cropPosition[axis]!=='number'||!Number.isFinite(next.cropPosition[axis])||next.cropPosition[axis]<0||next.cropPosition[axis]>1)))throw Error('Crop positions must be between 0 and 100 percent.');
  if(next.framing==='crop-custom'&&!next.cropPosition)throw Error('Choose the crop position for this clip.');
...
export function createTimelineEditor({ store, saveProject, onError = () => {}, onExport = () => {} }) {
  ...
  function inspectorPatch() {
    if(!node?.querySelector('#tleTrim'))return null;
    const framing=node.querySelector('#tleFraming').value||undefined;
    return {trimStart:Number(node.querySelector('#tleTrim').value),duration:Number(node.querySelector('#tleDuration').value),caption:node.querySelector('#tleClipCaption').value,muted:node.querySelector('#tleMuted').checked,framing,cropPosition:framing==='crop-custom'?{x:Number(node.querySelector('#tleCropX').value)/100,y:Number(node.querySelector('#tleCropY').value)/100}:undefined};
  }
  function inspectorChanged(clip=selected(),patch=inspectorPatch()) {
    if(!clip||!patch)return false;
    return Number(clip.trimStart||0)!==patch.trimStart||Number(clip.duration)!==patch.duration||String(clip.caption||'')!==patch.caption||Boolean(clip.muted)!==patch.muted||clip.framing!==patch.framing||JSON.stringify(clip.cropPosition)!==JSON.stringify(patch.cropPosition);
  }
  function applyInspectorChanges() {
    safe();const clip=selected(),patch=inspectorPatch();if(!inspectorChanged(clip,patch))return;
    const next=validateClipEdit(clip,patch,durationFor(clip),fps());
    mutate(clips().map(c=>c.id===clip.id?next:c),{captionChanged:patch.caption!==(clip.caption||'')});
  }
  function previewInspector() {
    safe();const clip=selected(),patch=inspectorPatch();if(!clip||!patch)return;
    stop();
    // The inspector edits the selected clip, even after playback/scrubbing moved on.
    if(locateTimelineTime(clips(),playhead)?.clip.id!==clip.id)loadAt(timelineSegments(clips()).find(s=>s.clip.id===clip.id).start,true);
    previewFraming({...clip,...patch});updateStatus();
  }
  ...
  function previewFraming(clip) {
    if(!node)return;const settings=store.project.settings||{},width=Number(settings.width)||1920,height=Number(settings.height)||1080,aspect=width/height;
    const framing=clip.framing||settings.framing||settings.rawEditProfile?.framing||(settings.fit==='cover'?'crop-center':'fit'),position=framing==='crop-custom'?clip.cropPosition||{x:.5,y:.5}:{x:framing==='crop-left'?0:framing==='crop-right'?1:.5,y:.5};
    const monitor=node.querySelector('.tle-monitor');if(monitor){monitor.style.width=aspect<1?'min(100%, 250px)':'100%';monitor.style.aspectRatio=String(aspect);}
    for(const media of node.querySelectorAll('#tleVideo,#tleImage')){media.style.height='100%';media.style.position='absolute';media.style.inset='0';media.style.objectFit=framing==='fit'?'contain':'cover';media.style.objectPosition=(position.x*100)+'% '+(position.y*100)+'%';}
  }
  function loadAt(position, forceSeek = false) {
    ...
    previewFraming(where.clip.id===selected()?.id&&inspectorChanged()?{...where.clip,...inspectorPatch()}:where.clip);const changed=loadedClip!==where.clip.id;if(changed){loadedClip=where.clip.id;video?.pause();if(video)video.hidden=where.clip.kind==='image';if(image)image.hidden=where.clip.kind!=='image';if(error)error.hidden=true;}
  ...
  async function save(exportAfter=false) {
    safe();const id=projectId;stop();
    try{applyInspectorChanges();saving=true;updateStatus();for(const clip of clips())validateClipEdit(clip,{},durationFor(clip),fps());const result=await saveProject();if(store.project?.id!==id)return;if(result===false||store.dirty)throw Error('Save this revision successfully before exporting.');revision=store.project.revision;history=[];if(exportAfter)await onExport();}
  ...
  function bind() {
    ...
    listen('#tleApply','click',applyInspectorChanges);
    listen('#tleFraming,#tleCropX,#tleCropY','input',previewInspector);
    listen('#tleTrim,#tleDuration,#tleClipCaption,#tleMuted','input',updateStatus);
  ...
  function updateStatus(){
    ...
    const status=node.querySelector('#tleSaveStatus');if(status)status.textContent=saving||store.saving?'Saving…':inspectorChanged()?'Unsaved clip changes · Save or Export to apply':store.dirty?'Unsaved changes':`Saved · revision ${store.project.revision}`;
    node.querySelectorAll('#tleSave,#tleExport,#tleApply,#tleSplit,#tleEarlier,#tleLater,#tleUndo,#tleTrim,#tleDuration,#tleClipCaption,#tleMuted,#tleFraming,#tleCropX,#tleCropY').forEach(el=>{if(saving||store.saving)el.disabled=true;else if(el.id==='tleUndo')el.disabled=!history.length;else if(el.id==='tleEarlier')el.disabled=selected()?.id===clips()[0]?.id;else if(el.id==='tleLater')el.disabled=selected()?.id===clips().at(-1)?.id;else if(el.id==='tleCropX'||el.id==='tleCropY')el.disabled=node.querySelector('#tleFraming')?.value!=='crop-custom';else el.disabled=!clips().length;});
  }
